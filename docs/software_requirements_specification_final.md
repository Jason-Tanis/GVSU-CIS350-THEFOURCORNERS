...
| STC2 | Verify that even if any previously created MySQL connection(s) under localhost in port 3306 (with username "root" and password "DegreeDollars350!") is deleted in MySQL Workbench, the tables Profile, Budgets, and Transactions are still created/loaded upon app startup. | In MySQL Workbench, click "Database", and then "Manage Connections". Delete any previously created connection(s) under localhost in port 3306 by selecting it in the "MySQL Connections" list and clicking "Delete". Close MySQL Workbench. Then, start Degree Dollars. | MySQL Workbench input: {delete any previously created connection(s) under localhost in port 3306} Degree Dollars input: ```briefcase run``` (inside the directory ```CIS350\GVSU-CIS350-THEFOURCORNERS\src\degree_dollars```} | Upon starting Degree Dollars, the message "Database and tables created successfully!" should print to the terminal. | When starting Degree Dollars, the message "Database and tables created successfully!" prints to the terminal. | Pass | HF4, ACCF2 (Note: this test case only checks for the existence of the three MySQL tables) |
| STC3 | Verify that no transactions are deleted from the Transactions table for a specific user, even when a large quantity of transactions are saved in immediate succession. This test will help in determining whether the database can handle large data quantities. | Log in, click "Create New Budget", select September 2025 for the month and year, add a subsection "Books" with amount 50.00, and click "Save Budget". Then, select September 2025 from the Home screen and click "Income/Expense +/-". In the Add Transaction window, add an expense from merchant "School" with amount 1.00 and click "Save Transaction". Add 49 more $1.00 expenses from merchant "School". | For budget creation: {Month: September, Year: 2025, Subsection: Books, Amount: 50.00} For transaction creation: {Income or Expense: Expense, Section: Education, Subsection: Books, Day of Month: 17, Amount: 1.00, Merchant: School}, repeat for 50 $1.00 transactions | On the History viewing screen for September 2025's budget, there should be 50 transaction summaries displayed, all of which being $1.00 expenses from School on September 17. | On the History viewing screen for September 2025's budget, there are 50 transaction summaries, all of which are $1.00 expenses from School on September 17. | Pass | HNF1 |
